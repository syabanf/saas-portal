...
export function AdminCommandPalette({
  ...
  const { tenants, users, applications, subscriptions, invoices, tenantsById, applicationsById } =
    useScoped()
  ...
  const results = [
    ...
    ...users.map((item) => ({
      ...
      label: item.name,
      meta: item.email,
      to: `/users?q=${encodeURIComponent(item.email)}`,
      icon: Users,
      group: 'User',
    })),
    ...
    ...subscriptions.map((item) => ({
      key: item.id,
      label: `${tenantsById.get(item.tenantId)?.name ?? 'Organization'} · ${applicationsById.get(item.applicationId)?.name ?? 'Application'}`,
    ...
    ...invoices.map((item) => ({
      key: item.id,
      label: item.number,
      meta: tenantsById.get(item.tenantId)?.name ?? 'Organization',
      to: `/billing/${item.id}`,
      icon: FileText,
      group: 'Invoice',
    })),
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="p-5">
        ...
        <SheetDescription className="text-muted mt-1 text-sm">
          Find organizations, users, applications, subscriptions and invoices.
        </SheetDescription>
        <Input
          autoFocus
          value={query}
          ...
          leftIcon={<Search />}
          placeholder="Type a name, email, code or invoice…"
          className="mt-5"
        />
          ...
          {results.length ? (
            results.map((item) => (
              <button
                ...
                className="hover:bg-surface flex w-full items-center gap-3 rounded-2xl p-3 text-left"
              >
            ...
            <p className="text-muted py-10 text-center text-sm">
              No matches. Try a name, email or code.
            </p>
          )}
        </div>
      </SheetContent>
